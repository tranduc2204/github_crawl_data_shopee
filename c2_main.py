...
#
def function():
    df = pd.DataFrame(columns=["timestamp", "name", "price", "price_min", "price_max", "price_min_before_discount", "price_max_before_discount"])
    x = 0
    for i in range (3):
        ...
        # Đẩy dữ liệu vào collection trong MongoDB 
        collection.insert_many(documents)
        
        # xóa hết trong dataframe để crawl trang sau không bị trùng data của trang trước
        df = df.drop(df.index).reindex(columns=["timestamp", "name", "price", "price_min", "price_max", "price_min_before_discount", "price_max_before_discount", "ID"])


while True:
    function()
    time.sleep(60)# 1p sẽ có 60s --> 10p là 600s
